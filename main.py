...
MODEL = None
HELMET_MODEL = None
HELMET_MODEL_PATH = BASE_DIR.parent / "helmet_best.pt"
    ...
    with MODEL_LOCK:
        if HELMET_MODEL is None:
            if not HELMET_MODEL_PATH.exists():
                raise RuntimeError(f"Helmet model not found: {HELMET_MODEL_PATH}")
...
def analyze_video(path: Path, progress) -> dict[str, Any]:
    ...
    while True:
        ...
        # Run the trained helmet model only when motorcycles are detected.
        ...
        if counts["motorcycle"] > 0:
            try:
                ...
                if helmet_results:
                    helmet_boxes = getattr(helmet_results[0], "boxes", None)
                    if helmet_boxes is not None and len(helmet_boxes) > 0:
                        helmet_classes = (
                            helmet_boxes.cls.cpu().numpy().astype(int)
                        )
                        frame_helmet_with = int(np.sum(helmet_classes == 0))
                        frame_helmet_without = int(np.sum(helmet_classes == 1))

                        helmet_with = max(helmet_with, frame_helmet_with)
                        helmet_without = max(helmet_without, frame_helmet_without)
                        max_helmet_violations = max(
                            max_helmet_violations,
                            frame_helmet_without,
                        )
            except Exception:
                # Vehicle analysis continues if helmet inference fails on a frame.
                pass
    ...
    result["helmet_detection"] = {
        "with_helmet": int(helmet_with),
        "without_helmet": int(helmet_without),
        "violations": int(max_helmet_violations),
        "model": HELMET_MODEL_PATH.name,
    }
    result["progress"] = 100
    return result
